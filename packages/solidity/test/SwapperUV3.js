...
describe("SwapperUV3Test", function () {
    ...
    async function testFixture() {
        let quoter2_addr, router_addr, pool_addr, weth_addr, usdc_addr, weth_whale, usdc_whale;
        const fee = 500; // 0.05% pool fee tier on Uniswap V3
        const maxSlippage = 25; // 0.25% slippage tolerance
        const Contract = await hardhat_1.default.ethers.getContractFactory("SwapperUV3Test");
        ...
        // //===============================
        const wethContract = new hardhat_1.default.ethers.Contract(weth_addr, ERC20_ABI_json_1.default, hardhat_1.default.ethers.provider);
        ...
        const usdcSigner = await hardhat_1.default.ethers.getSigner(usdc_whale);
        const usdcContract = new hardhat_1.default.ethers.Contract(usdc_addr, ERC20_ABI_json_1.default, usdcSigner);
        console.log(await usdcContract.balanceOf(weth_whale));
        await usdcContract.transfer(weth_whale, ethers_1.ethers.parseUnits("10000", 6));
        console.log(await usdcContract.balanceOf(weth_whale));
    ...
    it("should get the minimum output amount with slippage applied", async function () {
        const { swapper, quoter2_addr, router_addr, weth_addr, usdc_addr, pool_addr, fee, maxSlippage, owner, otherAccount } = await (0, network_helpers_1.loadFixture)(testFixture);
        const amountIn = ethers_1.ethers.parseUnits("100", 6); // 100 USDC
        ...
        (0, chai_1.expect)(amountOutMin).to.be.gt(0);
