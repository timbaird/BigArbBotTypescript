...
class ListenerTracker {
     ...
     */
  addListener(poolName: string, target: EventEmitter, event: string, listener: (...args: any[]) => void): void {
  ...
  async removeAllListeners(): Promise<void> {
      ...
      for (const [listenerName, listenerInfo] of this.listeners) {
            ...
            for (const { poolName, target, event, listener } of listenerInfo) {
                ...
                this.logger.log(
                    'info',
                    `ListenerTracker.removeAllListeners : ${poolName} pool has ${target.listenerCount(event)} listeners AFTER removal`
                );
            }
        }
